...
GAUSSIAN_OVERLAP_BATCH_PAIRS=${GAUSSIAN_OVERLAP_BATCH_PAIRS:-8192}
SKIP_GAUSSIAN_OVERLAP=${SKIP_GAUSSIAN_OVERLAP:-0}
...
echo "gaussian_overlap_batch_pairs: $GAUSSIAN_OVERLAP_BATCH_PAIRS"
...
if [[ "$SKIP_GAUSSIAN_OVERLAP" != "1" ]]; then
  uv run dalg-run-metrics gaussian-overlap \
    --data-dir "$RUN_DIR" \
    --out-dir "$OUT_DIR" \
    --device cuda \
    --batch-pairs "$GAUSSIAN_OVERLAP_BATCH_PAIRS"
fi
